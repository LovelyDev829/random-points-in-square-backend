...
// CREATE 
router.route('/update-frame').post((req, res, next) => {
    const { frameId, frame } = req.body
    console.log("update-frame", req.body)
    frameSchema.find({
        frame: frame
    }, (error, data) => {
        if (error) {
            ...
            var time = today.getHours() + ':' + today.getMinutes() + ':' + today.getSeconds();
            const commentDateTime = date + ' / ' + time;
            frameSchema.findByIdAndUpdate(frameId, {$set: {...req.body, commentDateTime:commentDateTime }}, (error, data) => {
                if (error) {
                    res.json({ success: false })
                } else {
                    console.log("Added a new pattern..")
